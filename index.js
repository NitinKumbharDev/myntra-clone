...
export const handelFilterClick = () => {
    return () => {
        newProduct = products.sort((a, b) => a.discountedPrice - b.discountedPrice)
...
async function createProductsList(scrollCount) {
    if (products.length) {
        ...
        const currentItems = products.slice(startIndex, endIndex) ?? newProduct.slice(startIndex, endIndex);
